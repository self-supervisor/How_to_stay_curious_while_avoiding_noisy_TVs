...
class ProcessFrame84(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        # np.save(f"obs_{unix_timestamp}.npy", obs)
...
class NoisyTVEnvWrapper(gym.Wrapper):
    def __init__(self, env):
        super(NoisyTVEnvWrapper, self).__init__(env)
        self.env._action_set = self.ale.getMinimalActionSet()
        self.noisy_action = self.env.action_space.n
        self.env._action_set = np.append(self.env._action_set, self.noisy_action)
        self.action_space = spaces.Discrete(len(self.env._action_set))

    def step(self, action):
        if action == self.noisy_action:
            action = 0
            obs, reward, done, info = self.env.step(action)
            obs = self.add_noisy_tv(obs)
        else:
            obs, reward, done, info = self.env.step(action)
        return obs, reward, done, info

    def add_noisy_tv(self, obs):
        # dealing with lazy frames business
        for i, a_frame in enumerate(obs._frames):
            obs._frames[i] = np.random.randint(
                low=0, high=256, size=obs._frames[0].shape, dtype="uint8"
            )
        return obs

class NoisyTVEnvWrapperCIFAR(NoisyTVEnvWrapper):
    def __init__(self, env, get_random_cifar):
        super().__init__(env) 
        self.get_random_cifar = get_random_cifar

    def grayscale(self, color_image):
        grayscale_image = color_image[:,:,0] * 0.2989 + color_image[:,:,1] * 0.5870 + 0.1140 * color_image[:,:,2]
        return grayscale_image

    def add_noisy_tv(self, obs):
        # dealing with lazy frames business
        for i, a_frame in enumerate(obs._frames):
            random_image = self.get_random_cifar()
            random_image = self.grayscale(random_image)
            tiled_image = np.tile(random_image, (3, 3)).astype('uint8')[:84, :84].reshape(84,84,1)
            save_tiled_image = random.choice([True] + 999 * [False])
            if save_tiled_image:
                plt.imshow(tiled_image[:,:,0])
                plt.savefig(f"tiled_image_{i}.png")
                plt.close()
                # np.save(f"tiled_image_{i}.npy", tiled_image)
            obs._frames[i] = tiled_image
        return obs

class NoisyTVEnvWrapperMario(gym.Wrapper):
    def __init__(self, env):
    ...
    def add_noisy_tv(self, obs):
        ...
        for i, a_frame in enumerate(obs._frames):
            obs._frames[i] = np.random.randint(
                low=128-64, high=128+64, size=obs._frames[0].shape, dtype="uint8"
            )
        return obs


class NoisyTVEnvWrapperMarioCIFAR(NoisyTVEnvWrapperMario):
    def __init__(self, env, get_random_cifar):
        super().__init__(env) 
        self.get_random_cifar = get_random_cifar
...
class ActionLoggingWrapper(gym.Wrapper):
    def __init__(self, env):
        super(ActionLoggingWrapper, self).__init__(env)
        self.action_counts = self.init_action_dict()
        self.intrinsic_reward_dict = self.init_action_dict()

    def step(self, action):
        obs, reward, done, info = self.env.step(action)
        self.action_counts[action] += 1
        self.intrinsic_reward_dict[action] += reward
        return obs, reward, done, info
    ...
    def get_average_intrinsic_reward_for_each_action(self):
        ...
        for action in range(self.action_space.n):
            action_dict[action] = (
                self.intrinsic_reward_dict[action] / self.action_counts
            )
        return action_dict
